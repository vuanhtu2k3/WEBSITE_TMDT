...
public class CartServiceImpl implements CartService {
	...
	public String addProductToCart(String userId, String prodId, int prodQty) {
		String status = "Không thể thêm vào giỏ hàng";
		...
		try {
			...
			if (rs.next()) {
				...
				if (prodQuantity > 0) {
					...
					if (k > 0)
						status = "Sản phẩm đã được xóa thành công khỏi giỏ hàng!";
				...
				status = "Sản phẩm không có trong giỏ hàng!";
		...
		try {
			...
			if (rs.next()) {
				...
				if (prodQty > 0) {
					...
					if (k > 0)
						status = "Sản phẩm được cập nhật thành công vào giỏ hàng!";
				} else if (prodQty == 0) {
					ps2 = con.prepareStatement("delete from usercart where username=? and prodid=?");
