...
public class AddtoCart extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (userName == null || password == null || usertype == null || !usertype.equalsIgnoreCase("customer")) {
			response.sendRedirect("login.jsp?message=Phiên đã hết hạn, hãy đăng nhập lại để tiếp tục!");
			return;
		}
		...
		if (pQty == cartQty) {
			...
			if (flag)
				status += "<br/>Sau đó, chúng tôi sẽ gửi thư cho bạn khi " + product.getProdName()
						+ " sẽ có sẵn trong Cửa hàng!";
